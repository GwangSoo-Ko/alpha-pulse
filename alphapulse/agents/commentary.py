...
class MarketCommentaryAgent:
    ...
    async def generate(self, pulse_result: dict, content_summaries: list[str],
                       feedback_context: str | None = None) -> str:
        """시장 해설 생성."""
        prompt = self._build_prompt(pulse_result, content_summaries, feedback_context=feedback_context)
        try:
